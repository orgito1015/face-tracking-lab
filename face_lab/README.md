# Face Recognition & Tracking Simulation Lab
...
- A webcam (built-in or USB). The scripts try index 0 then index 1 automatically.
...
| `rc_log.csv` | Generated by `pid_tracker.py` after a tracking session |

---

## Per-Member Quick-Run Guide

### Member 1 — Webcam Test

```bash
python webcam_test.py
```

Verify: green FPS counter and gold HH:MM:SS appear in the top-left corner.
Press **Q** to quit.

### Member 2 — Face Detection & Benchmark
...
Record your FPS results in `docs/member2/benchmark_table.md`.

### Member 3 — PID Tracker
...
...
```
Press **Q** to stop. `rc_log.csv` appears in the current directory.

### Member 4 — Analysis Plot
...
Answer the reflection questions in `docs/member4/reflection_answers.md`.

### Member 5 — Ethics Brief

Use the templates in `docs/member5/`:

- `joint_ethics_brief.md` — main deliverable
- `legal_framework_notes.md` — US/EU/Albania legal summary
- `stride_table.md` — fill in severity and mitigation columns
...
| Low FPS on Windows | The scripts already use `cv2.CAP_DSHOW` automatically |
